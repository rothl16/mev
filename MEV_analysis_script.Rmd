...
3. re-create netherlands data (currently only NA)
4. name Age_Group in all studies the same (ge = Age_Group, e.g. _sa = AgeG)
4.1 when solved, add back to reduction function
...
#df_ne <- preparation(df_ne) need new data simulation
...
#combine results
cg_results <- rbind(cg_all, cg_ge, cg_me, cg_sa) #ADD NE
cg_results$target <- rep("core_goodness",nrow(cg_results)) #add column to mention DV
rm(list=c("cg_all","cg_ge","cg_me","cg_sa")) #ADD NE; #removes non-needed dfs
```
...
#combine results
vc_results <- rbind(vc_all, vc_ge, vc_me, vc_sa) #ADD NE
vc_results$target <- rep("value_commitment",nrow(vc_results)) #add column to mention DV
rm(list=c("vc_all","vc_ge","vc_me","vc_sa")) #ADD NE; #removes non-needed dfs
```
...
#combine results
pd_results <- rbind(pd_all, pd_ge, pd_me, pd_sa) #ADD NE
pd_results$target <- rep("pay_deservingness",nrow(pd_results)) #add column to mention DV
rm(list=c("pd_all","pd_ge","pd_me","pd_sa")) #ADD NE; #removes non-needed dfs
```
...
#combine results
wa_results <- rbind(wa_all, wa_ge, wa_me, wa_sa) #ADD NE
wa_results$target <- rep("warmth",nrow(wa_results)) #add column to mention DV
rm(list=c("wa_all","wa_ge","wa_me","wa_sa")) #ADD NE; #removes non-needed dfs
```
...
#combine results
co_results <- rbind(co_all, co_ge, co_me, co_sa) #ADD NE
co_results$target <- rep("competence",nrow(co_results)) #add column to mention DV
rm(list=c("co_all","co_ge","co_me","co_sa")) #ADD NE; #removes non-needed dfs
```
...
#add names list
samples <- c("global","germany","mexico","south_africa") #ADD NE
