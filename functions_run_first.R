...
clean_data <- function(df, progress_threshold = 100, AC1_value = 7, AC2_value = 1) {
  filtered_df <- df %>%
    ...
    filter(Cou < 2) #residency != target country
...
#write function to only keep relevant columns
reduction <- function(df){
  reduced_df <- df %>% 
    select(c(
      ResponseId, ends_with("_lo"), ends_with("_he"),starts_with("FW"), ends_with("_diff"),
      effort_check,Age, Gen
    ))
  return(reduced_df)
}
...
plot_violin_with_means <- function(data, columns_to_pivot, y_label, plot_title = "") {
  ...
  y_min <- as.numeric(min(df_long$value)*0.9)
  y_max <- as.numeric(max(df_long$value)*1.1)
